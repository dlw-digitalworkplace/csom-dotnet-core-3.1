...
namespace Demo.CsomDotnetCore.WebApi.CSOM
{
    public class AuthenticationManagerUsernamePw : IDisposable
    {
        ...
        public AuthenticationManagerUsernamePw(string aadAppId)
        {
            _defaultAadAppId = aadAppId;
        }
        ...
        public ClientContext GetContext(Uri web, string userPrincipalName, string userPassword)
        {
            ...
            {
                string accessToken = EnsureAccessTokenAsync(new Uri($"{web.Scheme}://{web.DnsSafeHost}"), userPrincipalName, userPassword).GetAwaiter().GetResult();
        ...
        private async Task<string> AcquireTokenAsync(Uri resourceUri, string username, string password)
        {
            ...
            using (var stringContent = new StringContent(body, Encoding.UTF8, "application/x-www-form-urlencoded"))
            {
                ...
                var tokenResult = JsonSerializer.Deserialize<JsonElement>(result); // remember to add sufficient delegated permissions & to enable 'Treat application as a public client' on your AAD app that is being used
        ...
        {
            var token = new System.IdentityModel.Tokens.Jwt.JwtSecurityToken(accessToken); // uses Microsoft.AspNetCore.Authentication.JwtBearer
